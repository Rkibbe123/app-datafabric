...
class HealthcareManager(EDI):
    ...
    @classmethod
    def flatten_to_json2(cls, d):
        return {
            **{k: d.get(k) for k in list(d.keys()) if k not in ['EDI', 'FunctionalGroup', 'Transaction', 'Claims', 'trnx']},
            **d['EDI'],
            **d['FunctionalGroup'],
            **d['Transaction'],
            'Claims': [
    ...
    def flatten_to_json3(cls, d):
        ...
        # Extract header-level fields from first claim (if present)
        header_fields = {}
        if built_claims:
            header_fields = {
                'header_info': built_claims[0].get('header_info'),
                # 'date_info': built_claims[0].get('date_info'),
                'payee': built_claims[0].get('payee'),
                'payer': built_claims[0].get('payer'),
                'payment': built_claims[0].get('payment'),
                'provider_adjustments': built_claims[0].get('provider_adjustments')
            }
        # Remove header-level fields from each claim
        # cleaned_claims = [
        #     {k: v for k, v in claim.items() if k not in ['header_info', 'payee', 'payer', 'payment', 'provider_adjustments']}
        #     for claim in built_claims
        # ]

        cleaned_claims = [
            claim.get("claim", {})  # Extract only the inner claim dict
            for claim in built_claims
        ]
        ...
        return {
            ...
            **d['FunctionalGroup'],
            **d['Transaction'],
            **header_fields,        # Top-level columns
            'claims': cleaned_claims  # Array of claim-specific fields only
        }
    ...
    @classmethod
    def flatten2(cls, edi, *args, **kwargs):
        return [
            {
                **kwargs,
                'EDI': EDIManager.class_metadata(edi),
                'FunctionalGroup': EDIManager.class_metadata(fg),
                'Transaction': EDIManager.class_metadata(trnx),
                'Claims': cls.get_claims_locations(  # array instead of single claim
                    trnx.transaction_type,
                    trnx.data,
                    trnx
                ),
                'trnx': trnx
