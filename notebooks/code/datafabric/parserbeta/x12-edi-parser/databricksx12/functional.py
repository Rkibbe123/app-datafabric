...
class FunctionalGroup(EDI):
    ...
    def __init__(self, segments, delim_cls = AnsiX12Delim, strict_transactions = True):
        ...
        self.functional_identifier_code = self.fg.element(1)
        self.application_sender_code = self.fg.element(2)
        self.application_receiver_code = self.fg.element(3)
        self.creation_date = self.fg.element(4)
        self.creation_date_time_datetime = self.fg.element(5)
        self.group_control_number = self.fg.element(6)
        self.responsible_agency_code = self.fg.element(7)
        self.version_release_industry_id_code = self.fg.element(8)
